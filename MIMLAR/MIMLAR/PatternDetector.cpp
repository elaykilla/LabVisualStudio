...
/**
Training a matcher increases the matching speed. It is used to optimize the performance of the FlannBaseMatcher. 
It builds an index tree of train descriptors. 
*/
...
/** 
We detect more accurate pattern corners. We warp the input image using estimatedhomography to obtain a pattern. 
Then weobtain another homography  and another set of inlier features. The resultant precise homography willbe the matrix product of the first
and second homography.
*/
bool PatternDetector::findPattern(const cv::Mat& image, PatternTrackingInfo& info)
{
    ...
    if (1)
    {
		...
		//cv::imshow("Final matches", getMatchesImage(tmp, m_pattern.frame, m_queryKeypoints, m_pattern.keypoints, m_matches, 100));
		//cv::waitKey(1000);
    }
...
/** 
Cross Matching filter using Ratio Test. We perform a KNN-matching first with k=2. This returns 2 neatest descriptors for  each match.
The match is returned only if the ration between the first and second matches is big enough.
*/
...
/** 
We use the findHomography function of OpenCv. It uses the Random sample consensus (RANSAC) method. Homography transformations will bring points
from a pattern to the query image coordinate system. 
*/
