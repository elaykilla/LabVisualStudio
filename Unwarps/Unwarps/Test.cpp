...
int main(int argc, char** argv)
{
	...
	//For video
	CvMat* prevgray = 0, *gray =0, *image = 0;

	//For static images
	//Mat image,targetImage;
	...
	while( key != 'q' )
	{
		...
		//For Video
		//Mat targetImage = imread("picture1.bmp",CV_LOAD_IMAGE_GRAYSCALE);
		Mat targetImage = imread( argv[1],CV_LOAD_IMAGE_GRAYSCALE );

		//For Static pictures from Arguments
		/*image = cv::imread( argv[1], CV_LOAD_IMAGE_GRAYSCALE );
		targetImage = imread( argv[2],CV_LOAD_IMAGE_GRAYSCALE );*/
		...
		drawKeypoints(image,imageKeypoints,imageKeypoints1,Scalar::all(-1),DrawMatchesFlags::DEFAULT);
		...
		Mat img_matches;
		drawMatches( image, imageKeypoints, targetImage, targetImageKeypoints,
